...
public class fromcsvTodb {
     public static void crearTablaVideos(Connection conn) {
        ...
        String sql1 = "CREATE TABLE IF NOT EXISTS tiktok.video (" +
    ...
    public static void eliminarVideosCortaDuracion(Connection conn, float duracionMinima) {
        ...
        String sql = "DELETE FROM  tiktok.video WHERE duracion < ?";
    ...
    public static void insertarVideos(Connection conn, ArrayList<Video> listaVideos) {
        ...
        String sql = "INSERT INTO  tiktok.video (nombre_video, duracion, autor, likes, fecha_subida, etiqueta_1, etiqueta_2, etiqueta_3) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public static void actualizarLikesUsuarios(Connection conn, ArrayList<Integer> likes) {
        String sql = "UPDATE  tiktok.video SET likes = ? WHERE autor = ?";
    ...
    public static void actualizarLikesUsuariosProcedimiento(Connection conn, ArrayList<Integer> likes) {
        try{
            CallableStatement callableStatement = conn.prepareCall("{CALL tiktok.subida_likes(?, ?)}");
            // Recorrer el ArrayList de likes y actualizar para cada usuario
            for (int i = 0; i < likes.size(); i++) {
                String nombreUsuario = "@usuario" + (i + 1);  // Generar nombre de usuario
                int numeroLikes = likes.get(i);  // Obtener el número de likes para el usuario
    ...
    public static void main(String[] args) {
        try (Connection conn = DriverManager.getConnection("jdbc:mysql://database-1.ctiucrb7qmp3.us-east-1.rds.amazonaws.com:3306/", "admin", "passwordm2")) {
            ...
            actualizarLikesUsuariosProcedimiento(conn,likes);
        } catch (SQLException e) {
